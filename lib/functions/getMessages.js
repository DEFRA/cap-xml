...
module.exports.getMessages = (event, context, callback) => {
  ...
  let query = 'select * from cx.messages m inner join (select distinct on (fwis_code) id from cx.messages order by fwis_code, created desc) as l on l.id = m.id where m.expires > now()::timestamp order by m.created desc'
