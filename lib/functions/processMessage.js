...
module.exports.processMessage = async (event) => {
  try {
    ...
    const { message: redisMessage, query: rdsMessage } = message.putQuery(message, messageV2)
    // store the message in database and redis/elasticache
    await Promise.all([service.putMessage(rdsMessage), redis.set(redisMessage.identifier, redisMessage)])
